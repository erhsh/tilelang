...
import os
from .core import PassRecord, STATUS_COMPLETED, STATUS_FAILED, STATUS_SKIPPED
...
def generate_html(records: list[PassRecord], output_path: str):
    ...
    if records:
        ...
        auto_show_js = (
            ...
            "\n"
            "  // j/k keyboard navigation + Shift+E global expand\n"
            ...
            "  });\n"
            "\n"
            "  // Alignment click handler (capture phase, before P2 highlight)\n"
            ...
            "  }, true);\n"
            "\n"
            "  // Click line number to highlight row\n"
            "  document.addEventListener('click', function(e) {\n"
            "    var td = e.target.closest('td[data-side]');\n"
            "    if (!td) return;\n"
            "    var tr = td.closest('tr');\n"
